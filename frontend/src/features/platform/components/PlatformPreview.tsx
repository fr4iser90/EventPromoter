...
import type { PlatformData, PreviewItem, PreviewTargetsValue as TargetsValue } from '../types'

function PlatformPreview({ platform, content, isActive }: { platform: string; content: Record<string, unknown>; isActive: boolean }) {
  const { t, i18n } = useTranslation()
  const theme = useTheme()
  const [previewHtml, setPreviewHtml] = useState<string | null>(null)
  const [previewCss, setPreviewCss] = useState<string | null>(null)
  const [previewDimensions, setPreviewDimensions] = useState<{ width?: number; height?: number } | null>(null)
  const [multiPreviews, setMultiPreviews] = useState<PreviewItem[] | null>(null)
  const [activeTab, setActiveTab] = useState(0)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  
  // Use usePlatformMetadata for platform metadata (name, icon, color)
  const { platform: platformData, loading: metadataLoading, error: metadataError } = usePlatformMetadata(platform) as unknown as {
  ...
  const { schema } = usePlatformSchema(platform) as unknown as {
    schema?: {
      editor?: {
        blocks?: Array<{
          type?: string
  ...
  useEffect(() => {
    ...
    const resolveLocale = async () => {
      ...
      const templates = (content._templates as Array<{ targets?: TargetsValue }> | undefined) || []
      ...
      const targetsValue = (targetsBlock?.id ? content[targetsBlock.id] : undefined) as TargetsValue | undefined
      if (!targetsValue) {
        setPreviewLocale(locale)
        return
      }
      ...
      <Box sx={{ 
        ...
        {multiPreviews && multiPreviews.length > 0 ? (
          ...
          <Box>
            ...
            >
              {multiPreviews.map((preview: PreviewItem, index: number) => {
