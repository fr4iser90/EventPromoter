...
function FileSelectionBlock({ block, content, onChange, uploadedFileRefs }) {
  ...
  const compatibleFiles = useMemo(() => {
    ...
    return uploadedFileRefs.filter(file => {
  ...
  const handleFileInclusionToggle = (event) => {
  ...
  const isFileSelected = (fileId) => {
    ...
    return content[fileSelectionKey].some(f => f.id === fileId);
  };

  // Handler für Checkbox-Klick (Toggle Auswahl pro Datei)
  const handleToggleFileSelection = (file) => {
    const currentSelected = content?.[fileSelectionKey] || [];
    const isSelected = currentSelected.some(f => f.id === file.id);

    let newSelected;
    if (isSelected) {
      newSelected = currentSelected.filter(f => f.id !== file.id);
    } else {
          ...
          {compatibleFiles.length > 0 ? (
            <>
              ...
              <List dense sx={{ 
                ...
                {compatibleFiles.map((file, index) => (
                  <React.Fragment key={file.id}>
                    <ListItem
                      ...
                      sx={{ 
                        ...
                        opacity: !isFileSelected(file.id) && (content?.[fileSelectionKey]?.length >= (selectionLimit?.max || Infinity)) ? 0.5 : 1,
                        borderRadius: 1,
                        mb: 0.5
                      }}
                    >
                      <ListItemIcon sx={{ minWidth: 40 }}>
                        <Checkbox
                          ...
                          tabIndex={-1}
                          disableRipple
                          disabled={!isFileSelected(file.id) && (content?.[fileSelectionKey]?.length >= (selectionLimit?.max || Infinity))}
                        />
                      </ListItemIcon>
              ...
              {selectionLimit?.max && content?.[fileSelectionKey]?.length >= selectionLimit.max && (
