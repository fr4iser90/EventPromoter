...
function DynamicPanelWrapper({ platform }: { platform: string }) {
  ...
  const [fieldOptions, setFieldOptions] = useState<Record<string, Array<{ label: string; value: string }>>>({}) // Cache for dynamic field options
  ...
  const loadExistingValues = async (platformId: string) => {
        ...
        if (field.optionsSource) {
          try {
            ...
            const response = await axios({
              method: ((field.optionsSource as { method?: string }).method || 'GET') as
                | 'GET'
                | 'POST'
                | 'PUT'
                | 'PATCH'
  ...
  const handleFieldChange = async (sectionId: string, fieldName: string, value: unknown) => {
    // Find field definition to check for actions
    let fieldDef = null
    for (const section of panelConfig?.sections || []) {
      fieldDef = section.fields?.find((f: PanelField) => f.name === fieldName)
      if (fieldDef) break
    }
            ...
            {panelConfig.sections && panelConfig.sections.length > 0 ? (
              <Box>
                {panelConfig.sections.map((section) => {
                  ...
                  const fieldsWithOptions = (section.fields || []).map((field: PanelField) => {
