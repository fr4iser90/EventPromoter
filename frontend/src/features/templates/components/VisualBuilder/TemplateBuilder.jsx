 ...
 */
function TemplateBuilder({ platform, template, schema, onChange, onSave }) {
  ...
  useEffect(() => {
    if (schema?.template?.defaultStructure && template?.template) {
      const initialBlocks = schemaToBlocks(template.template, schema)
      setBlocks(initialBlocks)
    } else if (schema?.template?.defaultStructure) {
      // Neues Template - erstelle Blöcke aus Schema
      const initialBlocks = schemaToBlocks({}, schema)
      setBlocks(initialBlocks)
    }
  }, [schema, template])
  ...
  useEffect(() => {
    if (blocks.length > 0 && schema?.template?.defaultStructure) {
      const templateData = blocksToSchemaFormat(blocks, schema)
      ...
      if (template?.template) {
        Object.entries(template.template).forEach(([key, value]) => {
          const fieldSchema = schema.template.defaultStructure[key]
  ...
  const handleAddBlock = useCallback((fieldName) => {
    const fieldSchema = schema?.template?.defaultStructure[fieldName]
  ...
  // Block aktualisieren
  const handleUpdateBlock = useCallback((blockId, data) => {
  ...
  const handleInsertVariable = useCallback((blockId, variable) => {
