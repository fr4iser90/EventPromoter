...
const TemplateSelector = ({
  ...
  platform: string
  onSelectTemplate?: (template: TemplateRecord, parsed?: unknown, targets?: TargetsSelection | null, specificFiles?: string[]) => void
  currentContent?: string
  globalFiles?: Array<string | FileRef>
  ...
  const { schema } = usePlatformSchema(platform) as { schema?: Record<string, unknown> } // Load schema to check for targets block
  const { parsedData, uploadedFileRefs } = useStore() as unknown as { parsedData: Record<string, unknown> | null; uploadedFileRefs: FileRef[] }
  ...
  const editorSchema = (schema?.editor as { blocks?: Array<Record<string, unknown>>; platformId?: string } | undefined)
  ...
  const [targetsValue, setTargetsValue] = useState<TargetsSelection | null>(null) // Store targets selection
  const [specificFiles, setSpecificFiles] = useState<FileRef[]>([]) // NEW: Specific files for this run
  ...
  const loadPreview = useCallback(async (template: TemplateRecord) => {
    ...
    if (targetsValue) {
      ...
      if (targetsValue.templateLocale) {
        ...
        // Priority 2: Try to resolve locale from targets (Option B: Locale pro Target)
          const targetsBlock = (schema?.editor as { blocks?: Array<Record<string, unknown>> } | undefined)?.blocks?.find((block) => block.type === 'targets') as
            | { rendering?: { dataEndpoints?: Record<string, string> } }
            | undefined
          const dataEndpoints = targetsBlock?.rendering?.dataEndpoints || {}
        
        try {
          const individualTargets = targetsValue.individual ?? []
          const groupTargets = targetsValue.groups ?? []
          if (targetsValue.mode === 'individual' && individualTargets.length > 0) {
          ...
          } else if (targetsValue.mode === 'groups' && groupTargets.length > 0) {
  ...
  const handleTemplateSelect = async (template: TemplateRecord) => {
  ...
  const targetsBlock = (editorBlocks as any[]).find(block => block.type === 'targets')
  ...
  const handleToggleSpecificFile = (file: FileRef) => {
  ...
  const isGlobalFile = (fileId: string) => {
  ...
  const groupedTemplates = templates.reduce<Record<string, TemplateRecord[]>>((acc, template) => {
                      ...
                      <Card sx={{ mb: 2 }}>
                        <CardContent>
                          <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 'bold' }}>
                            {t(targetsBlock.label || 'template.targets')}
                          </Typography>
                          {targetsBlock.description && (
                            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                              {t(targetsBlock.description || 'template.targets')}
                            </Typography>
                          )}
                          <CompositeRenderer
                            block={targetsBlock}
                            value={targetsValue ?? undefined}
                            onChange={(value) => setTargetsValue(value as TargetsSelection)}
                            platform={platform}
                          />
                        </CardContent>
                      </Card>
                        ...
                        <CardContent sx={{ p: 0 }}>
                          ...
                          >
                            ...
                            <AccordionDetails sx={{ px: 2, pb: 2 }}>
                              ...
                              >
                                {uploadedFileRefs.map((file: FileRef) => {
