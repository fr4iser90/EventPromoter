...
import type { ApplyResults, PlatformTemplateInfo } from '../types'
...
function BulkTemplateApplier({ 
  ...
  onApply?: (platformId: string, content: Record<string, unknown>) => void
}) {
  const { t } = useTranslation()
  const { categories = [], loading: categoriesLoading } = useTemplateCategories()
  const [selectedCategory, setSelectedCategory] = useState('')
  const [selectedPlatformsForApply, setSelectedPlatformsForApply] = useState<Set<string>>(
    new Set(selectedPlatforms)
  )
  const [selectedTemplates, setSelectedTemplates] = useState<Record<string, string>>({}) // { platformId: templateId }
  ...
  const { templates, loading: templatesLoading } = useTemplatesByCategory(
    selectedCategory,
    selectedPlatforms
  ) as { templates: PlatformTemplateInfo[]; loading: boolean }

  const { parsedData, uploadedFileRefs } = useStore() as {
    parsedData: Record<string, unknown> | null
    uploadedFileRefs: Array<Record<string, unknown>>
  }
    ...
    if (templates.length > 0) {
      ...
      templates.forEach((t) => {
        if (t.hasTemplate && t.availableTemplates && t.availableTemplates.length > 0) {
          ...
          const selectedId = firstTemplate.templateId || firstTemplate.id
          if (selectedId) {
            autoSelected[t.platformId] = selectedId
          }
  ...
  const hasExistingContent = (platformId: string) => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      setApplyResults({
        ...
        errors: [{
          platformId: 'general',
          error: error instanceof Error ? error.message : t('template.failedToApplyTemplates')
        }]
      })
    } finally {
      setApplying(false)
