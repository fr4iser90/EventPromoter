...
type EditedData = {
  [key: string]: unknown
  extendedData?: Record<string, Record<string, unknown>>
  hashtags?: string[]
  title?: string
  date?: string
  time?: string
...
function Preview() {
  ...
  const { 
    ...
    uploadedFileRefs: EventFileRef[]
    parsedData: EditedData | null
    currentEvent: { id?: string } | null
    updateParsedData: (data: Record<string, unknown>) => Promise<boolean>
    debouncedSaveParsedData: (data: Record<string, unknown>) => void
    savingParsedData: boolean
    parsedDataSaveError: string | null
    lastParsedDataSave: string | null
    newEvent: () => void
    workflowState: string
  }
  const [selectedImage, setSelectedImage] = useState<EventFileRef | null>(null)
  const [selectedTextFile, setSelectedTextFile] = useState<EventFileRef | null>(null)
  const [textContent, setTextContent] = useState('')
  const [loadingText, setLoadingText] = useState(false)
  const [editedData, setEditedData] = useState<EditedData>(storeParsedData || {})
  ...
  const handleImageClick = (fileData: EventFileRef) => {
    setSelectedImage(fileData)
  }

  const handleTextFileClick = async (fileData: EventFileRef) => {
  ...
  const handleExtendedDataChange = (groupId: string, fieldId: string, value: unknown, isNested = false, parentFieldId: string | null = null) => {
    const updated: EditedData = { ...editedData }
    ...
    const groupData = updated.extendedData[groupId]

    if (isNested && parentFieldId) {
      // Nested field (e.g., ticketInfo.presale.price)
      if (!groupData[parentFieldId]) {
        groupData[parentFieldId] = {}
      }
      const parentData = groupData[parentFieldId] as Record<string, unknown>
      
      // Remove field if boolean is false or value is empty
      if (typeof value === 'boolean' && value === false) {
        delete parentData[fieldId]
        // Clean up empty parent object
        if (Object.keys(parentData).length === 0) {
          delete groupData[parentFieldId]
        }
        ...
        parentData[fieldId] = value
      }
    } else {
      // Simple field
      // Remove field if boolean is false or value is empty
      if (typeof value === 'boolean' && value === false) {
        delete groupData[fieldId]
      } else if (value === null || value === undefined || (typeof value === 'string' && value.trim() === '')) {
        delete groupData[fieldId]
      } else {
        groupData[fieldId] = value
      }
    }
  ...
  const getExtendedDataValue = (groupId: string, fieldId: string, parentFieldId: string | null = null) => {
  ...
  const hasGroupData = (groupId: string) => {
    const groupData = editedData?.extendedData?.[groupId]
  ...
  const updateHashtagsInStore = (newHashtags: string[]) => {
  ...
  const checkConditionalRequired = (groupId: string, parentFieldId: string | null) => {
    ...
    const priceText = typeof price === 'string' ? price : String(price ?? '')
    const urlText = typeof url === 'string' ? url : String(url ?? '')
    const hasPrice = priceText.trim().length > 0
    const hasUrl = urlText.trim().length > 0
  ...
  const renderField = (field: ExtendedField, groupId: string, parentFieldId: string | null = null) => {
    ...
    switch (field.type) {
      ...
      case 'tel':
        ...
        const displayedCurrency = isPriceField && value
          ? (extractCurrencyFromPrice(String(value)) || getDefaultCurrency(currentLocale))
                  ...
                  <Grid item xs={12} sm={6} md={4} key={fileData.id}>
                    <Card sx={{ cursor: 'pointer' }} onClick={() => handleImageClick(fileData)}>
                      <CardMedia
                        component="img"
                        height="200"
                        image={getFileUrl(fileData.url || '') || undefined}
                        alt={fileData.name}
                        sx={{ objectFit: 'cover' }}
                      />
                      <CardContent sx={{ py: 1 }}>
              ...
              <Card>
                <CardContent>
                  ...
                  <Grid container spacing={2}>
                    {Boolean(editedData.date) && (
                      <Grid item xs={12} sm={6} md={3}>
                        ...
                        <DateInput
                          fullWidth
                          size="small"
                          value={(editedData.date as string) || ''}
                    ...
                    {Boolean(editedData.time) && (
                      <Grid item xs={12} sm={6} md={3}>
                        ...
                        <TimeInput
                          fullWidth
                          size="small"
                          value={(editedData.time as string) || ''}
                    ...
                    {Boolean(editedData.venue || editedData.city) && (
                      <>
                        <Grid item xs={12} sm={6} md={3}>
                          <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
                            📍 {t('event.venue')}
                    ...
                    {Boolean(editedData.description) && (
                      <Grid item xs={12}>
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
                          📝 {t('event.description')}
                        </Typography>
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Box sx={{ minWidth: 0, flex: 1 }}>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {((fileData.size || 0) / 1024).toFixed(1)} KB
                          </Typography>
                        </Box>
                      </Box>
                    </Card>
        ...
        <Fade in={!!selectedImage || !!selectedTextFile}>
          ...
          >
            ...
            {selectedImage && (
              <Box>
                <img
                  src={getFileUrl(selectedImage.url || '') || undefined}
