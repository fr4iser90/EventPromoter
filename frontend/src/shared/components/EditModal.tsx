...
type SchemaAction = {
  id: string
  type: 'submit' | 'delete' | string
  endpoint?: string
  label: string
  ui?: { color?: 'primary' | 'secondary' | 'error' | 'info' | 'success' | 'warning'; variant?: 'text' | 'outlined' | 'contained' }
}
...
const EditModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = getApiUrl(schema?.endpoint || saveEndpoint || '')
      const response = await fetch(url, {
        method,
        headers: {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const deleteAction = schema.actions?.find((action: SchemaAction) => action.type === 'delete');
