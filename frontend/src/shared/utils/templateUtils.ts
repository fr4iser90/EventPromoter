...
export function getTemplateVariables(parsedData: GenericRecord = {}, uploadedFileRefs: GenericRecord[] = []): GenericRecord {
 ...
 */
export function replaceTemplateVariables(content: unknown, variables: GenericRecord): string | unknown {
 ...
 */
export function getTemplateVariableNames(parsedData: GenericRecord = {}, uploadedFileRefs: GenericRecord[] = []): string[] {
 ...
 */
export function getUnfulfilledVariables(content: unknown, availableVariables: GenericRecord = {}): string[] {
  if (!content || typeof content !== 'string') {
    return []
  }
 ...
 */
export function extractTemplateVariables(template: GenericRecord = {}): string[] {
  if (!template) return []
 ...
 */
export function isAutoFilledVariable(variableName: string, parsedData: GenericRecord = {}): boolean {
  if (!parsedData) return false

  // Map variable names to parsedData fields
  const autoFilledMap: Record<string, string> = {
    'date': 'date',
    'eventDate': 'date',
    'time': 'time',
    'eventTime': 'time',
  ...
  return Boolean(mappedField && parsedData[mappedField] !== undefined && parsedData[mappedField] !== null)
}
 ...
 */
export function getCanonicalVariableName(variableName: string): string {
 ...
 */
export function getVariableLabel(variableName: string): { label: string; icon: string } {
 ...
 */
export function extractVariablesFromContent(content: GenericRecord = {}): GenericRecord {
  if (!content) return {}
