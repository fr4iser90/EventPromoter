...
/**
 * Convert ISO date (YYYY-MM-DD) to formatted string based on locale
 * 
 * @param isoDate - ISO format date string (YYYY-MM-DD)
 * @param locale - Optional locale override (defaults to getUserLocale())
 * @returns Formatted date string (e.g., "16.05.2026" for de-DE)
 */
export const convertISOToFormatted = (isoDate: string, locale?: string): string => {
  if (!isoDate) return ''
  const userLocale = locale || getUserLocale()
  return formatDateForDisplay(isoDate, userLocale)
}

/**
 * Convert formatted date string to ISO format (YYYY-MM-DD)
 * Handles multiple formats: DD.MM.YYYY, MM/DD/YYYY, YYYY-MM-DD
 * 
 * @param dateString - Formatted date string (e.g., "16.05.2026" or "05/16/2026")
 * @returns ISO format date string (YYYY-MM-DD) or original string if parsing fails
 */
export const convertFormattedToISO = (dateString: string): string => {
  if (!dateString) return ''
  
  const date = parseDateString(dateString)
  if (!date) return dateString // Fallback: return as-is if parsing fails
